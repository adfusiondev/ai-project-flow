...
Locales are configured in `astro.config.mjs` (`locales`): English at the root (`/`) and Arabic at `/ar/`. The language selector switches between locales in both directions, and the navigation structure is identical for both. All section pages are translated into both English and Arabic.
